...
const Login: React.FC = () => {
  ...
  const validationSchema = Yup.object({
    email: Yup.string().required(t('GLOBAL.VALIDATION.required') as string),
    password: Yup.string().required(t('GLOBAL.VALIDATION.required') as string)
  });
  ...
  const onSubmit = async () => {
    ...
    const user = await fetch(`https://fakestoreapi.com/users/${randomUser}`).then(res => res.json());
